...
This is a bounded visual demo showing that frozen transformer hidden states can
act as a direct control signal for a toy app. A tiny language model reads a
command, a PyTorch hook captures the pre-lm-head hidden-state vector, and a
trained probe routes that vector to a deterministic VectorBot grid-world action.
...
VectorBot is a 5x5 grid-world controller with a small bounded action space:
...
The app does not ask the language model to generate a command. It treats the
model's activation vector as the software boundary and trains only a lightweight
projection layer on top of a frozen model.
...
action selected from hidden-state vectors.
...
The latent-space plot is a 2D PCA projection of hidden-state features colored by
action label. It is a visual diagnostic, not the classifier itself.
...
- Built a zero-generation latent-control demo where `distilgpt2` hidden states
  route natural-language commands to a bounded grid-world app through a trained
  probe.
- Implemented deterministic VectorBot state transitions, OOD abstention, route
  logging, ASCII rendering, static HTML replay, and generated GitHub visuals.
- Added CPU-friendly dataset, extraction, training, quickstart, and verification
  scripts with tests guarding no `model.generate()`, no generated command
  parsing, and no keyword/regex action router.
